            ...
            steps {
                script{
                  sh """
                     ...
                     export PUBLIC_IP2=\$(/usr/local/bin/aws ec2 describe-instances --filters "Name=instance-state-name,Values=running" --query 'Reservations[1].Instances[0].[PublicIpAddress]' --output text); \
